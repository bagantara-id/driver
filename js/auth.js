...
import { signInWithPopup, signOut, onAuthStateChanged, setPersistence, browserLocalPersistence } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-auth.js";
...
const aksiLogin = () => {
    goOnline(db);
    // Kunci token ke memori keras HP sebelum memanggil Popup Google
    setPersistence(auth, browserLocalPersistence)
        .then(() => {
            return signInWithPopup(auth, provider);
        })
        .catch(error => alert("Otorisasi Gagal: " + error.message, "error"));
};
...
onAuthStateChanged(auth, (user) => {
    if (user && user.email) {
        // PERBAIKAN: Paksa huruf kecil dan hapus spasi tersembunyi sebelum dicocokkan
        ...
        get(ref(db, `whitelist_emails/${safeEmail}`)).then((snap) => {
            if (snap.exists() && snap.val() === true) {
                ...
                // Ditolak (Tidak ada di database): Tendang instan ke Google
                signOut(auth).then(() => { window.location.replace("https://www.google.com"); });
            }
        }).catch(err => {
            // Ditolak (Error Firebase Rules): Tendang instan ke Google
